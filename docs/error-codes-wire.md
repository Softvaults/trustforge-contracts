...
1. Add the new variant in `contracts/trustforge_errors/src/lib.rs` in the correct category section.
...
4. Add or update tests in `contracts/trustforge_errors/tests/error_codes_wire.rs`.
5. Run `cargo test -p trustforge_errors error_codes_wire` to verify the discriminant values.
6. Review the new variant in `docs/errors.md` to keep the canonical reference in sync.

## Testing

The package includes a dedicated wire-stability assertion test in `contracts/trustforge_errors/tests/error_codes_wire.rs`.
