...
Cross-contract interactions (like `TrustForgeDelegation` calling `TrustForgeBond`) produce nested trees. This document outlines the threats associated with these trees and how the TrustForge protocol mitigates them.
...
**Mitigation**: The Soroban host ensures that if a function calls `require_auth`, the tree MUST contain a matching entry. If the entry is missing, the transaction fails. We ensure every security-critical mutation in `TrustForgeBond` is guarded by `require_auth`.
...
**Mitigation**: `TrustForgeDelegation` uses `owner.require_auth()`. This ensures that Alice explicitly authorized the specific call to `TrustForgeDelegation`. In a cross-contract scenario, the tree shape is `Alice -> TrustForgeDelegation -> TrustForgeBond`. Alice's signature covers this entire path.
...
**Mitigation**: Soroban's `require_auth` automatically includes the `contract_id` and `network_id` in the signature hash. Additionally, `TrustForgeDelegation` payloads explicitly include the `contract_id` for domain separation.
...
## Canonical Auth-Tree Shape for TrustForge

For a delegated action where a `Delegate` (Bob) performs a task on behalf of an `Owner` (Alice) using the `TrustForgeDelegation` contract to call `TrustForgeBond`:

```mermaid
graph TD
    A[Alice's Signature] --> B["TrustForgeDelegation::execute_delegated_revoke_attest(Alice, Subject, Payload)"]
    B --> C["TrustForgeBond::revoke_attestation(Alice, ID)"]
```

The Soroban host verifies that `Alice` authorized the call to `TrustForgeDelegation`, and that `TrustForgeDelegation` is authorized to call `TrustForgeBond` on Alice's behalf.
