...
//! without an explicit [`trustforge_delegation::CONTRACT_SPEC_VERSION`] bump.
//!
//! Refresh workflow (intentional ABI change only):
//! 1. `cargo build -p trustforge_delegation --target wasm32-unknown-unknown --release`
//! 2. Copy the new hex from test failure output (or `tests/spec_xdr/`) into
//!    `tests/spec_xdr/trustforge_delegation.v1.hex`.
...
const WASM_REL: &str = "target/wasm32-unknown-unknown/release/trustforge_delegation.wasm";
const EXPECTED_SPEC_XDR_HEX: &str = include_str!("spec_xdr/trustforge_delegation.v1.hex");
...
fn read_release_wasm() -> Vec<u8> {
    ...
    std::fs::read(wasm_path()).unwrap_or_else(|err| {
        panic!(
            "could not read {}: {err}. Run `cargo build -p trustforge_delegation \
...
fn contract_spec_xdr_is_pinned() {
    ...
    assert_eq!(
        ...
        "contractspecv0 XDR changed. If this ABI change is intentional, refresh \
         tests/spec_xdr/trustforge_delegation.v1.hex and bump \
         trustforge_delegation::CONTRACT_SPEC_VERSION. See tests/spec_xdr_regression.rs."
    );
}
...
fn spec_change_without_version_bump_is_rejected() {
    ...
    let stale_version = trustforge_delegation::CONTRACT_SPEC_VERSION.saturating_sub(1);
