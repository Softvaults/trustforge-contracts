...
All TrustForge smart contracts share a single error type: `ContractError`, defined
in the `trustforge_errors` crate. Every public entry-point returns
...
mapping is centrally maintained in `trustforge_errors::ErrorExt`; clients,
...
| 300-399  | Attestation    | trustforge_bond                               |
...
## Testing
```sh
cargo test -p trustforge_errors
```
