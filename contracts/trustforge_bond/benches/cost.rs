//! Gas-regression gate for the `trustforge_bond` contract.
//!
//! Run via `cargo bench -p trustforge_bond --features gas-bench --bench cost` (or
...
//! `cargo run -p trustforge_bond --bin update-cost-baseline`. See
...
fn main() -> ExitCode {
    ...
    let baseline_text = match std::fs::read_to_string(&baseline_path) {
        ...
        Err(_) => {
            eprintln!(
                "no baseline at {} — create one with `cargo run -p trustforge_bond --bin update-cost-baseline`",
    ...
    eprintln!(
        "\nIf this change is intended, refresh the baseline:\n  \
         cargo run -p trustforge_bond --bin update-cost-baseline"
    );
    ExitCode::FAILURE
}
