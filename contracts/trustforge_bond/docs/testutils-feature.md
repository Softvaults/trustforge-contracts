...
`trustforge_bond` as an `rlib` and need test-only helpers:

```toml
# In downstream Cargo.toml
[dev-dependencies]
trustforge_bond = { path = "...", features = ["testutils"] }
```
...
cargo build --target wasm32-unknown-unknown --release -p trustforge_bond

# Wrong — would bloat the WASM with test infrastructure
cargo build --target wasm32-unknown-unknown --release -p trustforge_bond --features testutils
```
...
- `contracts/trustforge_bond/Cargo.toml` — feature definitions
- `contracts/trustforge_bond/src/slash_history.rs` — `pub mod testutils` pattern
- `contracts/trustforge_bond/src/rolling_bond.rs` — `pub mod testutils` pattern
- `contracts/trustforge_bond/src/nonce.rs` — `mod testutils_helpers` pattern
