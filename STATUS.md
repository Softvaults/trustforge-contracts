...
| **Deployable at all?** | **`trustforge_bond` — no, for two independent reasons.** (1) Its release WASM is 137KB against Soroban's ~64KB on-chain limit (129KB as re-measured 2026-08-12), more than double budget even with maximal size optimization already applied — see [VERIFICATION.md](VERIFICATION.md) §9. (2) **More seriously: an internal code review found `withdraw()`, `withdraw_bond()`, and `collect_fees()` do not transfer any tokens to anyone** — they update accounting only, and the pull-payment claims system that could recover from this has no entrypoint to actually pay out a claim. See [docs/BOND_REVIEW_NOTE.md](docs/BOND_REVIEW_NOTE.md) — not yet fixed. The other 7 deployable contracts are within WASM budget and weren't in scope for this finding. |
