...
| **Deployable at all?** | **`trustforge_bond` — no, for one remaining reason.** Its release WASM is 131KB against Soroban's ~64KB on-chain limit (re-measured 2026-08-12), more than double budget even with maximal size optimization already applied — see [VERIFICATION.md](VERIFICATION.md) §9. A more serious issue found the same day — `withdraw()`, `withdraw_bond()`, and `collect_fees()` not transferring tokens, plus 4 related findings — was **fixed same-day**; see [docs/BOND_REVIEW_NOTE.md](docs/BOND_REVIEW_NOTE.md) for what was found and what changed, with new tests asserting real on-chain token-balance movement. The fix was not independently verified by a human before being applied. The other 7 deployable contracts are within WASM budget and weren't in scope for this finding. |
...
| `unwrap()`/`expect()`/`panic!()` in non-test contract code | **0** in the compiled surface of `trustforge_bond`, `trustforge_errors`, `trustforge_registry`, and `templates` (down from 950 at the 2026-08-07 baseline) — enforced by a crate-level `#[deny(clippy::unwrap_used, clippy::expect_used)]` so regressions fail CI. 3 documented exceptions remain, all provably-dead code with no production caller (see [docs/ORPHANED_MODULES.md](docs/ORPHANED_MODULES.md)). `trustforge_math` has ~11 by design — its checked-math helpers have no `Env` parameter so can't raise a typed `ContractError`; the 2 hottest call paths already have `Result`-returning counterparts. Tracked in [QUALITY_UPGRADE_ROADMAP.md](QUALITY_UPGRADE_ROADMAP.md) Phase 3. |
| Orphaned modules in `trustforge_bond` | **Resolved (2026-08-12): deleted.** 70 files (~19,000 lines) that were never declared as `mod` in `lib.rs` — 8 real feature modules (verifier staking, governance-voted slashing, evidence storage, access control, cooldowns, fees, status snapshots, liquidation scanning) and ~62 test files — have been removed rather than restored. Restoring would have meant adding ~45 new value-custody entrypoints outside design review. See [docs/ORPHANED_MODULES.md](docs/ORPHANED_MODULES.md) for the finding and resolution. |
| Unsafe code | **Zero in the repository**, compiled surface or otherwise (verified by manual grep; `cargo-geiger` itself wasn't run — see [VERIFICATION.md](VERIFICATION.md) §8). The only 2 `unsafe` blocks that existed were in the now-deleted orphaned test files above. |
